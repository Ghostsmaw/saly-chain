 ...
 *
 * Fail-open on transport errors: the token already passed cryptographic
 * verification, and an identity outage must not lock every operator out.
 * Only a definitive 401/403 from identity reports the session as revoked.
 */
...
export async function isSessionRevoked(token: string): Promise<boolean> {
  ...
  try {
    ...
    return false; // identity unreachable — trust the locally verified token
  }
