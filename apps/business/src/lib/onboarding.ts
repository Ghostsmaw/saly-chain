...
/** @deprecated Use isOnboardingEnabledForProfile('business') */
export async function isNewOnboardingFlowEnabled(): Promise<boolean> {
  return isOnboardingEnabledForProfile('business');
}
...
export async function startBusinessOnboarding(input: {
  userId: string;
...
export async function getBusinessOnboarding(userId: string): Promise<OnboardingStatusDto> {
  return compliance().getOnboarding(userId);
}

export async function submitBusinessOnboardingStep(
  userId: string,
  step: string,
...
export async function resubmitBusinessOnboarding(userId: string): Promise<OnboardingStatusDto> {
  return compliance().resubmitOnboarding(userId);
}

/** Read-only redirect target after auth — cookie sync must happen in a Server Action. */
