  ...
  return (
    <div className="saly-glass flex items-center gap-2 rounded-lg p-3">
      ...
      <div className="leading-tight">
        ...
        <p className="text-[11px] text-text-tertiary">All rails operational</p>
      </div>
    </div>
  );
}

function OrgChip({ orgName }: { orgName?: string }) {
  ...
  const name = user?.displayName || orgName || 'Your Business';
  const email = user?.email ?? 'Treasury account';
  const initials = name
    .split(/\s+/)
    .map((w) => w[0])
    .join('')
