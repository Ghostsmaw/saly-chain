...
export function OnboardingBanner({
  ...
  return (
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-text-primary">
            Complete your {label} verification to unlock the platform
          </p>
          <p className="mt-0.5 text-xs text-text-secondary">
            You currently have profile-only access. Treasury, payments, and team features are locked until verification is
            complete.
          </p>
      ...
      <Link
        href="/onboarding"
        className="shrink-0 rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-400"
      >
        Continue {label}
      </Link>
    </div>
...
export function VerificationReviewBanner({
  label,
  submittedAt,
}: {
  label: string;
  ...
  return (
    <div className="mb-6 flex flex-wrap items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3">
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-text-secondary">
          Your verification package is in the compliance queue. Platform features stay locked until a super-admin approves
          your account.
        </p>
        {submittedAt ? (
          <p className="mt-1 text-[11px] text-text-muted">Submitted {new Date(submittedAt).toLocaleString()}</p>
        ) : null}
      </div>
    </div>
  );
    ...
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs text-text-secondary">
            Update your documents and resubmit for review. Platform access remains locked until verification is approved.
          </p>
        </div>
      </div>
      <Link
  ...
  return (
    <div className="mx-auto max-w-lg rounded-2xl border border-amber-500/30 bg-amber-500/5 p-8 text-center">
      ...
      <p className="mt-2 text-sm text-text-secondary">
        Your {label} package was submitted successfully. A super-admin must approve your verification before you can access
        treasury, payments, and other platform features.
      </p>
      {submittedAt ? (
        <p className="mt-3 text-xs text-text-muted">Submitted {new Date(submittedAt).toLocaleString()}</p>
      ) : null}
      <p className="mt-4 text-xs text-text-tertiary">
        You can review your profile in{' '}
        <Link href="/settings" className="text-brand-300 hover:text-brand-200">
          Settings
        </Link>{' '}
        while you wait.
      </p>
...
export function VerificationRejectedDashboard({ label }: { label: string }) {
  return (
    <div className="mx-auto max-w-lg rounded-2xl border border-rose-500/30 bg-rose-500/5 p-8 text-center">
      ...
      <p className="mt-2 text-sm text-text-secondary">
        Your {label} submission was reviewed and could not be approved. Update your documents and resubmit for another
        review.
      </p>
      <Link
        href="/onboarding"
