...
/**
 * Agents-service transactional outbox store. `enqueueTx` writes an event row in
 * a caller's transaction (atomic with the state change); `enqueue` uses the base
 * connection (reliable, near-atomic). The relay drains rows to NATS.
 */
@Injectable()
export class OutboxService implements OutboxStore {
  ...
  async fetchPending(limit: number): Promise<OutboxRecord[]> {
    ...
    return rows.map((r) => ({ id: r.id, subject: r.subject, payload: r.payload, attempts: r.attempts }));
  }
