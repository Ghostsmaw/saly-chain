...
export class DomainErrorFilter implements ExceptionFilter {
  private readonly logger = new Logger(DomainErrorFilter.name);
    ...
    const correlationId = (req.headers['x-correlation-id'] as string | undefined) ?? error.correlationId;
    if (error.httpStatus >= 500) {
      this.logger.error(`${error.code} ${error.message}`, exception instanceof Error ? exception.stack : undefined);
    } else {
      this.logger.warn(`${error.code} ${error.message}`);
    }
    res.status(error.httpStatus).json({ error: { ...error.toJSON(), correlation_id: correlationId } });
  }
    ...
    if (exception instanceof HttpException) {
      ...
      const message =
        typeof response === 'string'
          ? response
          : ((response as { message?: string | string[] }).message as string) ?? exception.message;
