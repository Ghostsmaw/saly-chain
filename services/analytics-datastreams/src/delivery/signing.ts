...
/**
 * Datastreams webhook signing — identical wire shape to the webhooks service so
 * subscribers can reuse one verifier across both products.
 *
 * Wire shape:
...
export function generateSigningSecret(bytes: number): { secretHex: string; keyId: string } {
  return {
    ...
    keyId: `dssec_${randomBytes(6).toString('hex')}`,
  };
}
